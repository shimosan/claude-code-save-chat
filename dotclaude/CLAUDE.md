<!-- claude-library:begin — 共有ルール + 端末設定の管理ブロック。load/merge はこのブロックだけを更新する。
     ブロックより上 (ユーザー自由エリア) と下 (メモリ領域) には触れない。 -->
...
`CLAUDE.md` はファイル内の開始マーカーと終了マーカーで画定される管理ブロックを中心に、上から 3 ゾーンで構成する。
実マーカー以外の説明文には marker token を書かない。単純な文字列検索で境界を探す load/merge 実装が、説明文を境界と誤検出するのを避けるため。

1. **ユーザー自由エリア** — ファイル冒頭 〜 開始マーカーの直前。
   ユーザーが自由に書ける領域 (空でも可)。**load 不可侵**。
2. **管理ブロック (S + L)** — 開始マーカー 〜 終了マーカー間。
   - **共有ルール (S)**: 構造化された配布ルール。**load が master で更新する**。人間管理。
   - **端末設定 (L)**: ホスト情報・パス・vault 設定 (管理ブロック内の `## ホスト情報` 以降)。
     値は端末ローカル。merge 時は agent が既存値を読み取り保持/再適用する (現行と同じ動作)。
3. **メモリ領域 (M)** — 終了マーカー 〜 ファイル末尾 (EOF)。
   `#`・`/memory`・Claude の追記先。端末ローカル。**load 不可侵**。見出しレベルは不定でよい
   (ゾーンは終了マーカーで画定、追記は常に末尾＝M に落ちる)。
...
library のパスは `library_path` とし、管理ブロック内の「ホスト情報」(L) で定義する。

**配布対象は `CLAUDE.md` の管理ブロック内の共有ルール (S) と `dotclaude/commands/*.md` のみ**。library 内のその他は配布の往復対象外:
...
- `scripts/`: workflow の正本 (`save-chat-core.md` 等) と補助スクリプト。非配布 — 各端末の薄い皮 (save-chat / config-manager) が `library_path` 経由で実行時に読む。

### 端末名

現在の端末名は管理ブロック内の「ホスト情報」(L) で定義する。
端末名の一覧は `<library_path>/local/machines.md` で定義する。

### 配布 (load)

**load = library → ローカル。管理ブロックだけを更新する。** save (ローカル → library) は廃止。
ユーザー自由エリア (管理ブロックの上) とメモリ領域 (管理ブロックの下) は触らない。端末で生まれた設定・メモリは
library のマスターへ書き戻さない (個人情報を public git に上げないため)。端末間でメモリを
共有したい場合は別機構 (harvest, 別途・未実装) を使う。

対象は `CLAUDE.md` の管理ブロック内の共有ルール (S) と `dotclaude/commands/*.md`。
...
1. **diff**: 対応ファイルを `diff` して差分を提示する。
   ...
   diff -ru <library_path>/dotclaude/commands ~/.claude/commands
   ```
   ただし管理ブロック外 (ユーザー自由エリア・メモリ領域) と管理ブロック内の「ホスト情報」(L) の差分は無視 (端末固有で正)。
...
4. **実行**: 管理ブロック内の **共有ルール (S) のみ master で更新**。**端末設定 (L = ホスト情報) は既存値を保持**
   (必要なら agent が旧値を読み取り再適用)。**管理ブロック外 (自由エリア・メモリ) は絶対に触らない。**
   判断と最終指示は必ずユーザーが行う。
...
管理ブロック内の **「ホスト情報」(L)** のプレースホルダ (`<vault_path>` 等) を記入する。メモリ領域 (M) は空のまま。
以降の更新は上記「配布 (load)」手順 (既存ファイルがあれば diff 提示・判断はユーザー)。

## メモリ追記の運用

メモリは全て **メモリ領域 (M)** = 終了マーカー以降に追記する。
管理ブロック・ユーザー自由エリアは触らない。
...
vault の実パスおよびフォルダ構成は管理ブロック内の「ホスト情報」(L) を参照。
...
各フォルダの命名パターンは管理ブロック内の「ホスト情報」(L) を参照。
