...
def sha256_text(text):
    """文字列の sha256 を返す。None は None (= ファイル無し)。drift/nway での内容変化検知用。"""
    if text is None:
        return None
...
def main(P):
    ...
    # style.less は CSS/LESS の自由記述ファイルだが、config-manager の目的は
    # editor 設定を他端末で完全再現できる snapshot を残すことなので全文も採る。
    # sha256 は drift/nway で短く一致確認するための補助値。
    crossnote_style = read_text(P["crossnote_style"])
    snap["crossnote"] = {"mtime": mtime(P["crossnote_style"]),
                         "value": {"style.less": os.path.isfile(px("crossnote_style")),
                                   "style.less.content": crossnote_style,
                                   "style.less.sha256": sha256_text(crossnote_style)}}
