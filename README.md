...
まずこの library フォルダをその端末のクラウド同期先に用意し (clone か既存の同期フォルダ)、**その実パスを控える** (例 `~/Dropbox/library/claude`)。これが以降の `<library_path>`。`library_path` 自体はこのセットアップで初めて `~/.claude/CLAUDE.md` に記録されるので、新端末ではこの実パスを直接使う (まだ `<library_path>` は未定義)。

Claude Code を使う端末では、その実パスを渡して依頼する (**入口はこの README**):

> 「`<実パス>/README.md` を読んでセットアップして」

Claude がこの README を読み、次を順に整える (各詳細は下記の該当節)。司令塔はこの節:

1. **`~/.claude/CLAUDE.md`** (全ツール共通・必須): `dotclaude/CLAUDE.md` をコピーし、「ホスト情報」セクションのプレースホルダ (`library_path`=この実パス / `vault_path` / `ai_note_folder` 等) をこの端末の値で埋める (値はユーザーが提示)。具体手順は `dotclaude/CLAUDE.md` の「セットアップ」節。
2. **Claude Code commands**: `dotclaude/commands/*.md` (`save-chat` / `config-manager` / `chat-list`) を `~/.claude/commands/` へ。
3. **Codex** (使う端末のみ): `dotcodex/` を `~/.codex/` へ (`AGENTS.md` は既存ならマーカー付き block を diff/merge、`skills/*` はコピー)。→ 下記「Codex 版 global rules / skills」。
4. **Copilot** (使う端末のみ): `copilot/prompts/save-chat.prompt.md` を VS Code User prompts へ。→ 下記「Copilot 版 save-chat」。
5. **`chat-list` コマンド** (任意): 皮の展開 + PATH 設定 → 下記 [「`/chat-list` — 展開 (deploy)」](#展開-deploy)。
...
> **Claude Code 本体を使わない (Codex / Copilot 専用) 端末でも `~/.claude/CLAUDE.md` は必須**: `library_path` / `vault_path` 等の設定原典であり、save-chat の仕様本体 (`scripts/save-chat-core.md`) もここから解決する。その端末では手順 1 を手動で行ってから 3 / 4 を展開する。
...
python3 <library_path>/scripts/chat-list.py --path GPU        # WS を部分一致で指定 (rename/正規化分裂も束ねる)
...
各行は `#` / 開始時刻 / 由来 (`CC`/`CX`/`CU`/`CP` + surface、末尾 `*`=archived/hidden) / id / サイズ / タイトル(`--long` で model 列)。`--sort`(header 名キー: 共通 `start`/`end`/`size`、一覧 `title`、`--workspaces` `total`/`path`。既定 start・新しい順) と `--reverse` で並べ替え、`--path`/`--title`/`--grep`(両モード)で絞り込み(既定 部分一致・`--exact` で完全一致)、`--dump` で全文 (stdout / `--open` でエディタ buffer / `--json` で構造化。各メッセージに時刻併記) を取り出せる。オプションの正本は `--help`。
...
- **git の exec ビット誤検出 (`core.fileMode`)**: この library は `.git` ごとクラウド (Dropbox 等) で共有する運用。実行ビット付き (git mode `100755`) の `scripts/chat-list.py` 等を、exec ビットを持てない Windows の `git status` が「mode のみの modified」(内容差ゼロ・`old mode 100755 → new mode 100644`) と**誤検出**する。`git config core.fileMode false` で解消する (committed mode `100755` は index/tree に保持され、clone 先は +x 付きで取得。**両 OS で無害**)。`.git` を共有しているので、この設定は共有 `.git/config` 経由で全端末に効く (OS ごとには変えられないが false で問題ない)。
- **共有 `.git` の注意**: 上記のとおり `.git` (config/refs/index) をクラウド共有しているため、**2 台で同時に git 操作しない** (同時 commit/checkout は index/lock/ref 衝突や conflicted copy の原因)。1 台ずつ・同期が落ち着いてから操作する。「端末がおかしい」時はまず `.git` 内の `*conflicted copy*` / 古い `*.lock` を疑う。
