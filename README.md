...
`python3 <library_path>/scripts/chat-list.py …` と打つ代わりに、PATH 上の `chat-list` で直接呼べる。スクリプトは shebang (`#!/usr/bin/env python3`) 付き・標準ライブラリのみ・`library_path` 非依存なので、PATH 上のディレクトリ (例 `~/.local/bin`) に **symlink を張るだけ**:

```bash
ln -sf <library_path>/scripts/chat-list.py ~/.local/bin/chat-list   # PATH 上の任意の bin へ
chat-list --help                                                    # どこからでも実行
chat-list --ws hoge                                                 # 例
```

- スクリプトは実行ビット付き (git mode `100755`) で配布されるので、clone/pull 後そのまま symlink できる (要 `chmod +x` なし)。
- symlink 自体は**端末ローカル設定**(skin の deploy と同様、非配布・非 commit)。`~/.local/bin` が PATH に無ければ、自分の PATH 上の bin を使うか PATH に追加する。
- `chat-list` は呼び出し時の cwd を現在 WS とみなす (= そのフォルダの claude+codex 履歴)。
