...
policy, or this configuration library, read these files when available:
...
- Do not append to `~/.claude/CLAUDE.md` or `~/.claude/CLAUDE.local.md` unless
  the user explicitly asks for a memory or configuration update and approves
  the target scope.
...
For vault access, follow the folder privacy rules in `~/.claude/CLAUDE.md` and
the concrete paths and folder names in `~/.claude/CLAUDE.local.md`.
